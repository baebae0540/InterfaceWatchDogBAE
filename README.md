...
- 연속 미감지 디바운스(`FailureGraceCount`)로 일시적 오탐 시 알람 보류
- 트레이 앱 중복 실행 방지 (서버 전체 단일 인스턴스)
...
| `Erweka.FailureGraceCount`    | 연속 미감지 그레이스 — N회 연속 미감지 시에만 알람 (기본 2, 1 = 즉시) |
...
`Installer/Output/` 폴더에 `InterfaceWatchDog_Setup_v1.4.0.exe`가 생성됩니다.
